pub const KEY_INPUT_FILE: &str = "input_file";
pub const HELP_INPUT_FILE: &str = "Argument is path to input file";
pub const VALUE_INPUT_FILE: &str = "INPUT_FILE";
pub const LONG_ARG_INPUT_FILE: &str = "input-file";
pub const DEFAULT_VALUE_INPUT_FILE: &str = "-";

pub const KEY_OUTPUT_FILE: &str = "output_file";
pub const HELP_OUTPUT_FILE: &str = "Create an output file";
pub const VALUE_OUTPUT_FILE: &str = "OUTPUT_FILE";
pub const LONG_ARG_OUTPUT_FILE: &str = "output-file";

pub const KEY_KEYSIZE: &str = "keysize";
pub const HELP_KEYSIZE: &str = "Alter keysize";
pub const VALUE_KEYSIZE: &str = "KEYSIZE";
pub const LONG_ARG_KEYSIZE: &str = "keysize";
pub const DEFAULT_VALUE_KEYSIZE: &str = "256";

pub const KEY_TIMER: &str = "timer";
pub const HELP_TIMER: &str = "Show elapsed time";
pub const VALUE_TIMER: &str = "TIMER";
pub const LONG_ARG_TIMER: &str = "timer";

pub const KEY_RESULT_COUNTER: &str = "result_counter";
pub const HELP_RESULT_COUNTER: &str = "Show number of results";
pub const VALUE_RESULT_COUNTER: &str = "RESULT_COUNTER";
pub const LONG_ARG_RESULT_COUNTER: &str = "result-counter";

pub const KEY_THREAD_COUNT: &str = "thread_count";
pub const HELP_THREAD_COUNT: &str = "Number of threads";
pub const VALUE_THREAD_COUNT: &str = "THREAD_COUNT";
pub const LONG_ARG_THREAD_COUNT: &str = "thread-count";
pub const DEFAULT_VALUE_THREAD_COUNT: &str = "8";

pub const KEY_BASIC_OUTPUT: &str = "basic_output";
pub const HELP_BASIC_OUTPUT: &str = "Show only keys on stdout for piping";
pub const VALUE_BASIC_OUTPUT: &str = "BASIC_OUTPUT";
pub const LONG_ARG_BASIC_OUTPUT: &str = "basic-output";

pub const KEY_VERBOSE: &str = "verbose";
pub const HELP_VERBOSE: &str = "Show more detailed output on stdout";
pub const VALUE_VERBOSE: &str = "VERBOSE";
pub const LONG_ARG_VERBOSE: &str = "verbose";

pub const KEY_SUPPRESS_OUTPUT: &str = "suppress_output";
pub const HELP_SUPPRESS_OUTPUT: &str = "Suppress output on stdout";
pub const VALUE_SUPPRESS_OUTPUT: &str = "SUPPRESS_OUTPUT";
pub const LONG_ARG_SUPPRESS_OUTPUT: &str = "suppress-output";

pub const KEY_BYTE_COUNT: &str = "byte_count";
pub const HELP_BYTE_COUNT: &str = "Number of bytes from input file";
pub const VALUE_BYTE_COUNT: &str = "NUMBER_OF_BYTES";
pub const LONG_ARG_BYTE_COUNT: &str = "byte-count";
pub const DEFAULT_VALUE_BYTE_COUNT: &str = "0";

pub const KEY_ENTROPY_DELTA: &str = "entropy_delta";
pub const HELP_ENTROPY_DELTA: &str = "entropy delta of how many results will be recognized";
pub const VALUE_ENTROPY_DELTA: &str = "ENTROPY_DELTA";
pub const LONG_ARG_ENTROPY_DELTA: &str = "entropy-delta";
pub const DEFAULT_VALUE_ENTROPY_DELTA: &str = "0.2";

pub const KEY_BUFFERSIZE: &str = "buffer_size";
pub const HELP_BUFFERSIZE: &str = "use a buffer of a defined size to read large files (size in MB between 500 and 2000)";
pub const VALUE_BUFFERSIZE: &str = "BUFFERSIZE";
pub const LONG_ARG_BUFFERSIZE: &str = "buffer-size";
pub const DEFAULT_VALUE_BUFFERSIZE: &str = "2000";
